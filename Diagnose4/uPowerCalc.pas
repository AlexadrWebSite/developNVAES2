...
interface
uses uStringOperation,SysUtils,matrices,uDLL,uCalcParamInfo,uDPZ,uNetSend;
...
const NullPhysValue:TPhysValue=(Value:0;Error:0;Status:1;Index:-1);
...
procedure NullNDPZ(var Ndpz:TNDPZ);

procedure CalcNdpz(var Ndpz:TNdpz;IDPZ:TIDPZ);
...
var i,j:integer;
begin
  Ndpz.Value:=NullPhysValue;
  for i:=1 to 54 do
   for j:=1 to 7 do
   Ndpz.QED[i][j]:=NullPhysValue;
  Ndpz.KDPZ:=NullPhysValue;

end;

procedure CalcNdpz(var Ndpz:TNdpz;IDPZ:TIDPZ);
var i,j,SumDost:integer;
    SumQED:double;
begin
 SumDost:=0;
 SumQED:=0;
 for i:=1 to 54 do
  for j:=1 to 7 do
    ...
    begin
     inc(SumDost);
     SumQED:=SumQED+Ndpz.QED[i][j].Value;
   end;
 if SumDost>0 then
  begin
   SumQED:=SumQED/SumDost;
   Ndpz.Value.Value:= Ndpz.KDPZ.Value*3.75*163*SumQED;
   Ndpz.Value.Status:=Status_OK;
  end else
  NDpz.Value.Status:=Status_Invalid;
