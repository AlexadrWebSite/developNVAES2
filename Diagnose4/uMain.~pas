...
//----------------------------------------------------------------------------------------
// Последнее изменение: Преобразование TBuffer в считывание всего среза сразу.
//----------------------------------------------------------------------------------------
...
var
  fMain: TfMain;
  Txt:TextFile;
   IniFile:TIniFile; // основной настроечный файл Diagnose.ini
...
begin
 ...
 Buffer:=TBuffer.Create(IniFile.ReadInteger('Buffer','Length',5));
 DecimalSeparator:='.';
 NPPUnit:=TNPPUnit.Create;
 TimeScheduler:=TTimeScheduler.Create(IniFile.ReadInteger('TimeScheduler','Interval',20));
 PTKZPanel[1]:=Panel1;
 PTKZPanel[2]:=Panel2;
 PTKZPanel[3]:=Panel3;
 PTKZPanel[4]:=Panel4;
...
//-----------------чтение через библиотеку
 c:=0;
  ...
  for i:=0 to Buffer.CountParam-1 do
   if  Buffer.CalcValues[i].Index<>-1 then
    begin
     val.Value:= GetFloat(clientId,getId,Buffer.CalcValues[i].Index);
     if GetStatus(clientId,getId,Buffer.CalcValues[i].Index)<> D_STATUS_UNDEF then
      val.Status:=Status_Ok else Val.Status:=Status_Invalid;
      Val.DateTime:=now;
      Buffer.AddValue(Buffer.CalcValues[i].Index, val);
...
//  CalcRegime; // определить режим
 ...
 if TimeScheduler.AutoCalc(checkState) then
  ...
  rtN10_100:begin
              ...
              N.Calculate(TC,GenTech,Buffer,NIni,IDPZ);
...
with fWelcome do
  begin
   ...
   fWelcome.ProgressBar.Max:=Buffer.CountParam div 10;
//   MessageDlg('Max='+InttoStr(fWelcome.ProgressBar.Max),mtInformation,[mbOk],0);
   application.processmessages;

   for i:=0  to Buffer.CountParam-1 do
    begin
     Buffer.CalcValues[i].Index:=Insert(clientId,getId,Pchar(string(Buffer.CalcValues[i].KKS)),0,0);
   ...
   begin
    ...
    if (i div 2)=(i/2) then
     begin
      lInfo.Caption:='Не удается подписаться по списку параметров '+inttoStr(round(i/2))+' секунд';
      SendMessage(fWelcome.Handle,WM_PAINT,0,0);
      application.ProcessMessages;
     end; 
   end;
...
begin
 ...
 if not NPPUnit.Initalize(ExtractFilePath( Application.ExeName)+'Files\NPPUnit.ini')
...
var DPZini:TIniFile;
begin
  DPZini:=TIniFile.Create( IniFile.ReadString('Nini','FileName','\Files\N.ini'));
  fWelcome.lInfo.Caption:='Привязка токов ДПЗ к расчетным значениям в буфере...';

  IDPZ:=TIDPZ.Create(DPZini,IniFile);
